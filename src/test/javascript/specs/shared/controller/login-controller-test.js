...
describe("LoginController", function() {
  ...
  it('should not login and show error when server returns error', function() {
    ...
    httpBackend.when('POST', '/auth/oauth/token?grant_type=password&username=admin&password=password').respond(401);
  ...
  it('should clear password on successful login attempt', function() {
    ...
    spyOn(messageService, 'populate');
    httpBackend.when('POST', '/auth/oauth/token?grant_type=password&username=admin&password=password').respond(200, {
