...
describe('dateUtils', function() {
    ...
    describe('addDaysToDate', function() {
        ...
        it('should add days', function() {
            var date = this.dateUtils.addDaysToDate(new Date('2017-05-10'), 2);
    ...
    describe('toDate', function() {
        ...
        it('should return undefined if parameter is null', function() {
            expect(this.dateUtils.toDate(null)).toEqual(undefined);
        });

        it('should create date from iso string', function() {
            var date = new Date('2017-05-12');

            expect(this.dateUtils.toDate(date.toISOString())).toEqual(date);
        });

        it('should remove time zone offset', function() {
            var date = new Date('2017-01-31T10:12:14');

            expect(this.dateUtils.toDate('2017-01-31T10:12:14')).toEqual(date);
        });

        it('should create date from array with 3 elements', function() {
            var date = new Date('2017-05-12');

            expect(this.dateUtils.toDate([2017, 5, 12])).toEqual(date);
        });

        it('should create date from array with 6 elements', function() {
            var date = new Date('2017-01-31T10:12:14Z');

            expect(this.dateUtils.toDate([2017, 1, 31, 10, 12, 14])).toEqual(date);
        });

        it('should return undefined if array has number of elements other that 3 or 6', function() {
            expect(this.dateUtils.toDate([])).toEqual(undefined);
            expect(this.dateUtils.toDate([1])).toEqual(undefined);
            expect(this.dateUtils.toDate([1, 2])).toEqual(undefined);
            expect(this.dateUtils.toDate([1, 2, 3, 4])).toEqual(undefined);
            expect(this.dateUtils.toDate([1, 2, 3, 4, 5])).toEqual(undefined);
            expect(this.dateUtils.toDate([1, 2, 3, 4, 5, 6, 7])).toEqual(undefined);
            expect(this.dateUtils.toDate([1, 2, 3, 4, 5, 6, 7, 8])).toEqual(undefined);
        });
    });
    ...
    describe('toArray', function() {
        ...
        it('should convert to date without time', function() {
            ...
            expect(this.dateUtils.toArray(date, false)).toEqual([2017, 5, 10]);
        });

        it('should convert to date with time', function() {
            var date = new Date('2017-05-08T10:12:14Z');

            expect(this.dateUtils.toArray(date, true)).toEqual([2017, 5, 8, 10, 12, 14]);
        });
    });
    ...
    describe('convertEpochMilliToIsoDateString', function() {
        ...
        it('should return ISO formatted date given epoch in milliseconds', function() {
            expect(this.dateUtils.convertEpochMilliToIsoDateString(1514793600000)).toEqual('2018-01-01T08:00:00.000Z');
        });
    });
